...
Vec3 Vec3MulQ(Vec3 a, Vec4 quat) {
    return Vec4MulQ(Vec4FromV3(a), quat).XYZ;
}
Vec3 Vec3MulM4(Vec3 a, Mat4x4 m) {
    return Vec4MulM4(Vec4FromV3(a), m).XYZ;
}
...
Vec4 QuatInv(Vec4 q) {
    ...
    return Vec4New(-q.x / mag2, -q.y / mag2, -q.z / mag2, q.w / mag2);
}
...
Vec4 Vec4MulQ(Vec4 a, Vec4 q) {
    return QuatMul(q, QuatMul(a, QuatInv(q)));
}
